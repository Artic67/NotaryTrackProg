...
export function writePDF (obj, folderToSave, filename, sourcefile) {
    ...
    let pdfWriter = hummus.createWriterToModify(path.resolve(__dirname,`pdf/${sourcefile}`), {
    ...
    for (let i = 0; i < pagesCount[sourcefile]; i++) {
        ...
        for (let key in obj) {
            ...
            if (coords[sourcefile][key]['pageNumber'] == i) {
                ...
                if (coords[sourcefile][key]['center']) length = font.calculateTextDimensions(obj[key], size).width;
                
                console.log(font.calculateTextDimensions(obj[key], size).width);
                
                context.getContext().writeText(
                    obj[key],
                    coords[sourcefile][key]['x']-(length/2), coords[sourcefile][key]['y'],
